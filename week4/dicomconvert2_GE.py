...
def convert_dicoms(subjs, dicom_dir_template, outdir, heuristic_file, converter, queue=None):
    for sid in subjs:
        if queue:
            ...
            outcmd = 'bsub -J psb6351-%s -q %s -e /scratch/PSB6351_2017/week4/salo/crash/dcm_err_%s -o /scratch/PSB6351_2017/week4/salo/crash/dcm_out_%s < %s' % (sid, queue, sid, sid, script_file)
            os.system(outcmd)
            continue
